...
bool ReadRichImageToAnnotatedMaskDatum(const string& filename,
    const string& labelname, const string& maskfile, const int height, const int width,
    const int min_dim, const int max_dim, const bool is_color,
    const std::string& encoding, const AnnotatedMaskDatum_AnnotationType type,
    const string& labeltype, const std::map<string, int>& name_to_label,
    AnnotatedMaskDatum* anno_datum);

inline bool ReadRichImageToAnnotatedMaskDatum(const string& filename,
    const string& labelname, const string& maskfile, const int height, const int width,
    const bool is_color, const std::string & encoding,
    const AnnotatedMaskDatum_AnnotationType type, const string& labeltype,
    const std::map<string, int>& name_to_label, AnnotatedMaskDatum* anno_datum) {
  return ReadRichImageToAnnotatedMaskDatum(filename, labelname, maskfile, height, width, 0, 0,
                      is_color, encoding, type, labeltype, name_to_label,
                      anno_datum);
}
