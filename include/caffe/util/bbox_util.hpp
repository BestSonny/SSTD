...
typedef map<int, vector<NormalizedOrientedBBox> > LabelOrientedBBox;
...
// Project Oriented bbox onto the coordinate system defined by src_bbox.
...
// Decode a bbox according to a prior bbox.
void DecodeOrientedBBox(const NormalizedBBox& prior_bbox,
    const vector<float>& prior_variance, const CodeType code_type,
    const bool variance_encoded_in_target, const bool clip_bbox,
    const NormalizedOrientedBBox& bbox, NormalizedOrientedBBox* decode_bbox);

// Decode a set of bboxes according to a set of prior bboxes.
void DecodeOrientedBBoxes(const vector<NormalizedBBox>& prior_bboxes,
    const vector<vector<float> >& prior_variances,
    const CodeType code_type, const bool variance_encoded_in_target,
...
// Decode all bboxes in a batch.
void DecodeOrientedBBoxesAll(const vector<LabelOrientedBBox>& all_loc_pred,
    const vector<NormalizedBBox>& prior_bboxes,
    const vector<vector<float> >& prior_variances,
    const int num, const bool share_location,
    const int num_loc_classes, const int background_label_id,
...
// Retrieve bounding box ground truth from oriented_gt_data and gt_data.
//    oriented_gt_data: 1 x 1 x num_gt x 5 blob.
...
// Get location predictions from oriented_loc_data.
//    loc_data: num x num_preds_per_class * num_loc_classes * 5 blob.
//    num: the number of images.
//    num_preds_per_class: number of predictions per class.
//    num_loc_classes: number of location classes. It is 1 if share_location is
//      true; and is equal to number of classes needed to predict otherwise.
//    share_location: if true, all classes share the same location prediction.
//    oriented_loc_preds: stores the location prediction, where each item contains
//      location prediction for an image.
template <typename Dtype>
...
// Encode the oriented localization prediction and ground truth for each matched prior.
//    all_oriented_loc_preds: stores the location prediction, where each item contains
//      location prediction for an image.
//    all_oriented_gt_bboxes: stores ground truth bboxes for the batch.
//    all_match_indices: stores mapping between predictions and ground truth.
//    prior_bboxes: stores all the prior bboxes in the format of NormalizedBBox.
//    prior_variances: stores all the variances needed by prior bboxes.
//    multibox_loss_param: stores the parameters for MultiBoxLossLayer.
//    oriented_loc_pred_data: stores the location prediction results.
//    oriented_loc_gt_data: stores the encoded location ground truth.
template <typename Dtype>
