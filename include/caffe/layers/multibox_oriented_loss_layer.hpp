#ifndef CAFFE_MULTIBOX_ORIENTED_LOSS_LAYER_HPP_
#define CAFFE_MULTIBOX_ORIENTED_LOSS_LAYER_HPP_
...
class MultiBoxOrientedLossLayer : public LossLayer<Dtype> {
 public:
  explicit MultiBoxOrientedLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "MultiBoxOrientedLoss"; }
  ...
  // bottom[4] stores the ground truth oriented bounding boxes.
  // bottom[5] stores the oriented location predictions.
 ...
 protected:
  ...
  shared_ptr<Layer<Dtype> > oriented_loss_layer_;
  LocLossType oriented_loss_type_;
  float oriented_weight_;
  // bottom vector holder used in Forward function.
  vector<Blob<Dtype>*> oriented_bottom_vec_;
  // top vector holder used in Forward function.
  vector<Blob<Dtype>*> oriented_top_vec_;
  // blob which stores the matched location prediction.
  Blob<Dtype> oriented_pred_;
  // blob which stores the corresponding matched ground truth.
  Blob<Dtype> oriented_gt_;
  // localization loss.
  Blob<Dtype> oriented_loss_;
