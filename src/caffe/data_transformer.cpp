...
DataTransformer<Dtype>::DataTransformer(const TransformationParameter& param,
    Phase phase)
    : param_(param), phase_(phase), force_mirror_flag_(false), mirror_flag_(false){
...
template<typename Dtype>
void DataTransformer<Dtype>::setForceMirrorFlag(bool force_mirror_flag){
  force_mirror_flag_ = force_mirror_flag;
}

template<typename Dtype>
void DataTransformer<Dtype>::setMirror(bool mirror_flag){
  mirror_flag_ = mirror_flag;
}

template<typename Dtype>
bool DataTransformer<Dtype>::getMirror(){
  bool flag =  param_.mirror() && Rand(2);
  return flag;
}
...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  if (force_mirror_flag_){
    *do_mirror = mirror_flag_;
  }else{
...
template<typename Dtype>
void DataTransformer<Dtype>::Transform(
    const AnnotatedMaskDatum& anno_datum, Blob<Dtype>* transformed_blob,
    RepeatedPtrField<AnnotationGroup>* transformed_anno_group_all, bool mask) {
  bool do_mirror;
  Transform(anno_datum, transformed_blob, transformed_anno_group_all,
            &do_mirror, mask);
}
...
void DataTransformer<Dtype>::Transform(
    const AnnotatedMaskDatum& anno_datum, Blob<Dtype>* transformed_blob,
    RepeatedPtrField<AnnotationGroup>* transformed_anno_group_all,
    bool* do_mirror, bool mask) {
  // Transform datum.
  const Datum& datum = anno_datum.datum();
  const Datum& mask_datum = anno_datum.mask();
  NormalizedBBox crop_bbox;
  if(mask == false){
    Transform(datum, transformed_blob, &crop_bbox, do_mirror);
  }else{
    Transform(mask_datum, transformed_blob, &crop_bbox, do_mirror);
  }
  // Transform annotation.
...
void DataTransformer<Dtype>::Transform(
    const AnnotatedMaskDatum& anno_datum, Blob<Dtype>* transformed_blob,
    vector<AnnotationGroup>* transformed_anno_vec, bool* do_mirror, bool mask) {
  RepeatedPtrField<AnnotationGroup> transformed_anno_group_all;
  Transform(anno_datum, transformed_blob, &transformed_anno_group_all,
            do_mirror, mask);
  for (int g = 0; g < transformed_anno_group_all.size(); ++g) {
...
void DataTransformer<Dtype>::Transform(
    const AnnotatedMaskDatum& anno_datum, Blob<Dtype>* transformed_blob,
    vector<AnnotationGroup>* transformed_anno_vec, bool mask) {
  bool do_mirror;
  Transform(anno_datum, transformed_blob, transformed_anno_vec, &do_mirror, mask);
}

template<typename Dtype>
void DataTransformer<Dtype>::TransformAnnotation(
    const AnnotatedMaskDatum& anno_datum,
    const NormalizedBBox& crop_bbox, const bool do_mirror,
    RepeatedPtrField<AnnotationGroup>* transformed_anno_group_all) {
  if (anno_datum.type() == AnnotatedMaskDatum_AnnotationType_BBOX) {
    // Go through each AnnotationGroup.
    for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
      const AnnotationGroup& anno_group = anno_datum.annotation_group(g);
      AnnotationGroup transformed_anno_group;
      ...
      for (int a = 0; a < anno_group.annotation_size(); ++a) {
        ...
        // Adjust bounding box annotation.
        NormalizedBBox proj_bbox;
        NormalizedOrientedBBox proj_oriented_bbox;
        if (ProjectBBox(crop_bbox, bbox, &proj_bbox) &&
            ProjectOrientedBBox(crop_bbox, oriented_bbox,&proj_oriented_bbox)) {
          has_valid_annotation = true;
          Annotation* transformed_anno =
              transformed_anno_group.add_annotation();
          transformed_anno->set_instance_id(anno.instance_id());
          ...
          NormalizedOrientedBBox* transformed_oriented_bbox = transformed_anno->mutable_oriented_bbox();
          transformed_oriented_bbox->CopyFrom(proj_oriented_bbox);
          if (do_mirror) {
            Dtype temp = transformed_oriented_bbox->xc();
            transformed_oriented_bbox->set_xc(1 - temp);
            temp = transformed_oriented_bbox->radians();
            transformed_oriented_bbox->set_radians(-1.0 * temp);
          }
        }
      }
      // Save for output.
...
template<typename Dtype>
void DataTransformer<Dtype>::CropImage(const AnnotatedMaskDatum& anno_datum,
                                       const NormalizedBBox& bbox,
                                       AnnotatedMaskDatum* cropped_anno_datum,
                                       bool mask) {
  // Crop the datum.
  if(mask){
    CropImage(anno_datum.mask(), bbox, cropped_anno_datum->mutable_mask());
  }
  else{
    CropImage(anno_datum.datum(), bbox, cropped_anno_datum->mutable_datum());
    // Transform the annotation according to crop_bbox.
...
void DataTransformer<Dtype>::Transform(Blob<Dtype>* input_blob,
  ...
  if(force_mirror_flag_){
    ...
    mirror = param_.mirror() && Rand(2);
  }
  const bool do_mirror = mirror;
  const bool has_mean_file = param_.has_mean_file();
  const bool has_mean_values = mean_values_.size() > 0;
