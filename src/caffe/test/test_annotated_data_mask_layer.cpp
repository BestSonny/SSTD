...
class AnnotatedDataMaskLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  AnnotatedDataMaskLayerTest()
  ...
  void Fill(DataParameter_DB backend, bool unique_pixel, bool unique_annotation,
            bool use_rich_annotation, AnnotatedMaskDatum_AnnotationType type) {
    backend_ = backend;
    ...
    for (int i = 0; i < num_; ++i) {
      ...
      Datum* mask_datum = anno_datum.mutable_mask();
      mask_datum->set_channels(channels_);
      mask_datum->set_height(height_);
      mask_datum->set_width(width_);
      std::string* mask_data = mask_datum->mutable_data();
      for (int j = 0; j < size_; ++j) {
        int elem = unique_pixel ? j : i;
        mask_data->push_back(static_cast<uint8_t>(elem));
      }
      // Fill annotation.
      if (use_rich_annotation) {
        ...
        for (int g = 0; g < i; ++g) {
          ...
          for (int a = 0; a < g; ++a) {
            ...
            if (type == AnnotatedMaskDatum_AnnotationType_BBOX) {
  ...
  void TestRead() {
    ...
    TransformationParameter* mask_param =
        param.mutable_mask_param();
    mask_param->set_scale(scale);

    AnnotatedDataMaskLayer<Dtype> layer(param);
    ...
    EXPECT_EQ(blob_top_mask_->num(), num_);
    EXPECT_EQ(blob_top_mask_->channels(), channels_);
    EXPECT_EQ(blob_top_mask_->height(), height_);
    EXPECT_EQ(blob_top_mask_->width(), width_);
    ...
    for (int iter = 0; iter < 5; ++iter) {
      ...
      for (int i = 0; i < num_; ++i) {
        if (use_rich_annotation_) {
          if (type_ == AnnotatedMaskDatum_AnnotationType_BBOX) {
            EXPECT_EQ(blob_top_label_->num(), 1);
            EXPECT_EQ(blob_top_label_->channels(), 1);
      ...
      for (int i = 1; i < num_; ++i) {
        for (int j = 0; j < size_; ++j) {
          EXPECT_EQ(scale * (unique_pixel_ ? j : i),
                    blob_top_mask_->cpu_data()[i * size_ + j])
              << "debug: iter " << iter << " i " << i << " j " << j;
        }
      }
    }
  ...
  void TestReshape(DataParameter_DB backend, bool unique_pixel,
                   ...
                   AnnotatedMaskDatum_AnnotationType type) {
    ...
    for (int i = 0; i < num_; ++i) {
      ...
      if (use_rich_annotation) {
        Datum* mask_datum = anno_datum.mutable_mask();
        mask_datum->set_channels(channels_);
        mask_datum->set_height(i % 2 + 1);
        mask_datum->set_width(i % 4 + 1);
        std::string* mask_data = mask_datum->mutable_data();
        const int mask_data_size =
            mask_datum->channels() * mask_datum->height() * mask_datum->width();
        for (int j = 0; j < mask_data_size; ++j) {
          mask_data->push_back(static_cast<uint8_t>(j));
        }
        anno_datum.set_type(type);
        for (int g = 0; g < i; ++g) {
          AnnotationGroup* anno_group = anno_datum.add_annotation_group();
    ...
    for (int iter = 0; iter < 3; ++iter) {
      ...
      EXPECT_EQ(blob_top_mask_->height(), iter % 2 + 1);
      EXPECT_EQ(blob_top_mask_->width(), iter % 4 + 1);
      // Check label.
      const Dtype* label_data = blob_top_label_->cpu_data();
      if (use_rich_annotation) {
        if (type == AnnotatedMaskDatum_AnnotationType_BBOX) {
        ...
        for (int h = 0; h < height; ++h) {
          for (int w = 0; w < width; ++w) {
            ...
            EXPECT_EQ(idx, static_cast<int>(blob_top_data_->cpu_data()[idx]))
                << "debug: iter " << iter << " c " << c
                << " h " << h << " w " << w;
            EXPECT_EQ(idx, static_cast<int>(blob_top_mask_->cpu_data()[idx]))
                << "debug: iter " << iter << " c " << c
                << " h " << h << " w " << w;
          }
        }
  ...
  void TestReadCrop(Phase phase) {
    ...
    EXPECT_EQ(blob_top_mask_->num(), num_);
    EXPECT_EQ(blob_top_mask_->channels(), channels_);
    EXPECT_EQ(blob_top_mask_->height(), 1);
  ...
  void TestReadCropTrainSequenceSeeded() {
    ...
    TransformationParameter* mask_param =
        param.mutable_mask_param();
    mask_param->set_crop_size(1);
    mask_param->set_mirror(true);

    // Get crop sequence with Caffe seed 1701.
    Caffe::set_random_seed(seed_);
    vector<vector<Dtype> > crop_sequence;
    {
      AnnotatedDataMaskLayer<Dtype> layer1(param);
    ...
    Caffe::set_random_seed(seed_);
    AnnotatedDataMaskLayer<Dtype> layer2(param);
  ...
  virtual ~AnnotatedDataMaskLayerTest() {
  ...
  AnnotatedMaskDatum_AnnotationType type_;
};

TYPED_TEST_CASE(AnnotatedDataMaskLayerTest, TestDtypesAndDevices);

#ifdef USE_LEVELDB
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadLevelDB) {
  const AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReshapeLevelDB) {
  const AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
  for (int p = 0; p < kNumChoices; ++p) {
    bool unique_pixel = kBoolChoices[p];
    for (int r = 0; r < kNumChoices; ++r) {
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTrainLevelDB) {
  ...
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
  this->Fill(DataParameter_DB_LEVELDB, unique_pixel, unique_annotation,
             use_rich_annotation, type);
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTrainSequenceSeededLevelDB) {
  const bool unique_pixel = true;  // all pixels the same; images different
  const bool unique_annotation = false;  // all anno the same; groups different
  const bool use_rich_annotation = false;
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTrainSequenceUnseededLevelDB) {
  const bool unique_pixel = true;  // all pixels the same; images different
  const bool unique_annotation = false;  // all anno the same; groups different
  const bool use_rich_annotation = false;
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTestLevelDB) {
  const bool unique_pixel = true;  // all pixels the same; images different
  const bool unique_annotation = false;  // all anno the same; groups different
  const bool use_rich_annotation = false;
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
...
#ifdef USE_LMDB
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadLMDB) {
  const AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
  for (int p = 0; p < kNumChoices; ++p) {
    bool unique_pixel = kBoolChoices[p];
    for (int r = 0; r < kNumChoices; ++r) {
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReshapeLMDB) {
   const AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
   for (int p = 0; p < kNumChoices; ++p) {
     bool unique_pixel = kBoolChoices[p];
     for (int r = 0; r < kNumChoices; ++r) {
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTrainLMDB) {
  const bool unique_pixel = true;  // all pixels the same; images different
  const bool unique_annotation = false;  // all anno the same; groups different
  const bool use_rich_annotation = false;
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
...
// Test that the sequence of random crops is consistent when using
// Caffe::set_random_seed.
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTrainSequenceSeededLMDB) {
  const bool unique_pixel = true;  // all pixels the same; images different
  const bool unique_annotation = false;  // all anno the same; groups different
  const bool use_rich_annotation = false;
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
...
// Test that the sequence of random crops differs across iterations when
// Caffe::set_random_seed isn't called (and seeds from srand are ignored).
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTrainSequenceUnseededLMDB) {
  const bool unique_pixel = true;  // all pixels the same; images different
  const bool unique_annotation = false;  // all anno the same; groups different
  const bool use_rich_annotation = false;
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
...
TYPED_TEST(AnnotatedDataMaskLayerTest, TestReadCropTestLMDB) {
  const bool unique_pixel = true;  // all pixels the same; images different
  const bool unique_annotation = false;  // all anno the same; groups different
  const bool use_rich_annotation = false;
  AnnotatedMaskDatum_AnnotationType type = AnnotatedMaskDatum_AnnotationType_BBOX;
