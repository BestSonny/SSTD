...
#include "caffe/layers/multibox_oriented_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void MultiBoxOrientedLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // Set up oriented localization loss layer.
  oriented_weight_ = multibox_loss_param.oriented_weight();
  oriented_loss_type_ = multibox_loss_param.loc_loss_type();
  // fake shape.
  vector<int> oriented_shape(1, 1);
  oriented_shape.push_back(5);
  oriented_pred_.Reshape(oriented_shape);
  oriented_gt_.Reshape(oriented_shape);
  oriented_bottom_vec_.push_back(&oriented_pred_);
  oriented_bottom_vec_.push_back(&oriented_gt_);
  oriented_loss_.Reshape(loss_shape);
  oriented_top_vec_.push_back(&oriented_loss_);
  if (oriented_loss_type_ == MultiBoxLossParameter_LocLossType_L2) {
    LayerParameter layer_param;
    layer_param.set_name(this->layer_param_.name() + "_l2_oriented_loc");
    layer_param.set_type("EuclideanLoss");
    layer_param.add_loss_weight(oriented_weight_);
    oriented_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
    oriented_loss_layer_->SetUp(oriented_bottom_vec_, oriented_top_vec_);
  } else if (oriented_loss_type_ == MultiBoxLossParameter_LocLossType_SMOOTH_L1) {
    LayerParameter layer_param;
    layer_param.set_name(this->layer_param_.name() + "_smooth_L1_oriented_loc");
    layer_param.set_type("SmoothL1Loss");
    layer_param.add_loss_weight(oriented_weight_);
    oriented_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
    oriented_loss_layer_->SetUp(oriented_bottom_vec_, oriented_top_vec_);
...
template <typename Dtype>
void MultiBoxOrientedLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(num_priors_ * loc_classes_ * 5, bottom[5]->channels())
      << "Number of priors must match number of oriented location predictions.";
  CHECK_EQ(num_priors_ * num_classes_, bottom[1]->channels())
      << "Number of priors must match number of confidence predictions.";
}

template <typename Dtype>
void MultiBoxOrientedLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const Dtype* loc_data = bottom[0]->cpu_data();
  const Dtype* conf_data = bottom[1]->cpu_data();
  const Dtype* prior_data = bottom[2]->cpu_data();
  const Dtype* gt_data = bottom[3]->cpu_data();
  const Dtype* oriented_gt_data = bottom[4]->cpu_data();
  const Dtype* oriented_loc_data = bottom[5]->cpu_data();
  ...
  map<int, vector<NormalizedOrientedBBox> > all_oriented_gt_bboxes;
  GetGroundTruth(oriented_gt_data, gt_data, num_gt_, background_label_id_, use_difficult_gt_,
                &all_oriented_gt_bboxes);
  ...
  GetLocPredictions(oriented_loc_data, num_, num_priors_, loc_classes_, share_location_,
                    &all_oriented_loc_preds);
  ...
  if (num_matches_ >= 1) {
    ...
    vector<int> loc_shape(2);
    loc_shape[0] = 1;
    loc_shape[1] = num_matches_ * 5;
    oriented_pred_.Reshape(loc_shape);
    oriented_gt_.Reshape(loc_shape);
    Dtype* oriented_pred_data = oriented_pred_.mutable_cpu_data();
    Dtype* oriented_gt_data = oriented_gt_.mutable_cpu_data();
    EncodeOrientedLocPrediction(all_oriented_loc_preds, all_oriented_gt_bboxes, all_match_indices_,
                        prior_bboxes, prior_variances, multibox_loss_param_,
                        oriented_pred_data, oriented_gt_data);
    oriented_loss_layer_->Reshape(oriented_bottom_vec_, oriented_top_vec_);
    oriented_loss_layer_->Forward(oriented_bottom_vec_, oriented_top_vec_);
  } else {
    oriented_loss_.mutable_cpu_data()[0] = 0;
  }
  ...
  if (this->layer_param_.propagate_down(5)) {
    Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
        normalization_, num_, num_priors_, num_matches_);
    top[0]->mutable_cpu_data()[0] +=
        oriented_weight_ * oriented_loss_.cpu_data()[0] / normalizer;
  }
}

template <typename Dtype>
void MultiBoxOrientedLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  // Back propagate on location prediction.
  if (propagate_down[5]) {
    Dtype* oriented_bottom_diff = bottom[5]->mutable_cpu_diff();
    caffe_set(bottom[5]->count(), Dtype(0), oriented_bottom_diff);
    if (num_matches_ >= 1) {
      vector<bool> oriented_propagate_down;
      // Only back propagate on prediction, not ground truth.
      oriented_propagate_down.push_back(true);
      oriented_propagate_down.push_back(false);
      oriented_loss_layer_->Backward(oriented_top_vec_, oriented_propagate_down,
                                oriented_bottom_vec_);
      // Scale gradient.
      Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
          normalization_, num_, num_priors_, num_matches_);
      Dtype oriented_weight = top[0]->cpu_diff()[0] / normalizer;
      caffe_scal(oriented_pred_.count(), oriented_weight, oriented_pred_.mutable_cpu_diff());
      // Copy gradient back to bottom[5].
      const Dtype* oriented_pred_diff = oriented_pred_.cpu_diff();
      int count = 0;
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int> >::iterator it =
             all_match_indices_[i].begin();
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            // Copy the diff to the right place.
            int start_idx = loc_classes_ * 5 * j + label * 5;
            caffe_copy<Dtype>(5, oriented_pred_diff + count * 5,
                              oriented_bottom_diff + start_idx);
            ++count;
          }
        }
        oriented_bottom_diff += bottom[5]->offset(1);
      }
    }
  }
...
INSTANTIATE_CLASS(MultiBoxOrientedLossLayer);
REGISTER_LAYER_CLASS(MultiBoxOrientedLoss);
