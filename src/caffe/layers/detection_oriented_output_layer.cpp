...
void DetectionOrientedOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void DetectionOrientedOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  if (need_save_) {
  ...
  // [image_id, label, confidence, xmin, ymin, xmax, ymax, xc, yc, width, height, radians]
  top_shape.push_back(12);
  top[0]->Reshape(top_shape);
}

template <typename Dtype>
void DetectionOrientedOutputLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      if (decode_bboxes.find(loc_label) == decode_bboxes.end() ||
          decode_oriented_bboxes.find(loc_label) == decode_oriented_bboxes.end()) {
        // Something bad happened if there are no predictions for current label.
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        top_data[count * 12 + 2] = scores[idx];
        const NormalizedBBox& bbox = bboxes[idx];
        const NormalizedOrientedBBox& oriented_bbox = oriented_bboxes[idx];
        top_data[count * 12 + 3] = bbox.xmin();
        top_data[count * 12 + 4] = bbox.ymin();
        top_data[count * 12 + 5] = bbox.xmax();
        top_data[count * 12 + 6] = bbox.ymax();
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(DetectionOrientedOutputLayer, Forward);
#endif

INSTANTIATE_CLASS(DetectionOrientedOutputLayer);
REGISTER_LAYER_CLASS(DetectionOrientedOutput);
