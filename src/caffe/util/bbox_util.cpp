...
bool ProjectOrientedBBox(const NormalizedBBox& src_bbox, const NormalizedOrientedBBox& bbox,
                NormalizedOrientedBBox* proj_bbox){
  if (bbox.xc() >= src_bbox.xmax() || bbox.xc() <= src_bbox.xmin() ||
      bbox.yc() >= src_bbox.ymax() || bbox.yc() <= src_bbox.ymin()) {
    return false;
  }
  float src_width = src_bbox.xmax() - src_bbox.xmin();
  float src_height = src_bbox.ymax() - src_bbox.ymin();
  proj_bbox->set_xc((bbox.xc() - src_bbox.xmin()) / src_width);
  proj_bbox->set_yc((bbox.yc() - src_bbox.ymin()) / src_height);
  proj_bbox->set_width(bbox.width() / src_width);
  proj_bbox->set_height(bbox.height() / src_height);
  proj_bbox->set_radians(bbox.radians());
  return true;
}
...
void EncodeOrientedBBox(const NormalizedBBox& prior_bbox,
    const vector<float>& prior_variance, const CodeType code_type,
    const bool encode_variance_in_target, const NormalizedOrientedBBox& bbox,
    NormalizedOrientedBBox* encode_bbox) {
  CHECK_EQ(code_type, PriorBoxParameter_CodeType_CENTER_SIZE);
  float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
  CHECK_GT(prior_width, 0);
  float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
  CHECK_GT(prior_height, 0);
  ...
  float bbox_width = bbox.width();
  CHECK_GT(bbox_width, 0);
  float bbox_height =  bbox.height();
  CHECK_GT(bbox_height, 0);
  float bbox_center_x = bbox.xc();
  float bbox_center_y = bbox.yc();
  float bbox_radians = bbox.radians();

  if (encode_variance_in_target) {
    encode_bbox->set_xc((bbox_center_x - prior_center_x) / prior_width);
    encode_bbox->set_yc((bbox_center_y - prior_center_y) / prior_height);
    encode_bbox->set_width(log(bbox_width / prior_width));
    encode_bbox->set_height(log(bbox_height / prior_height));
    encode_bbox->set_radians(bbox_radians);
  } else {
    // Encode variance in bbox.
    encode_bbox->set_xc(
        (bbox_center_x - prior_center_x) / prior_width / prior_variance[0]);
    encode_bbox->set_yc(
        (bbox_center_y - prior_center_y) / prior_height / prior_variance[1]);
    encode_bbox->set_width(
        log(bbox_width / prior_width) / prior_variance[2]);
    encode_bbox->set_height(
        log(bbox_height / prior_height) / prior_variance[3]);
    encode_bbox->set_radians(bbox_radians);
  }
}
...
void DecodeOrientedBBox(
    const NormalizedBBox& prior_bbox, const vector<float>& prior_variance,
    const CodeType code_type, const bool variance_encoded_in_target,
    const bool clip_bbox, const NormalizedOrientedBBox& bbox,
    NormalizedOrientedBBox* decode_bbox) {
  CHECK_EQ(code_type, PriorBoxParameter_CodeType_CENTER_SIZE);
  float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
  CHECK_GT(prior_width, 0);
  float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
  ...
  float decode_bbox_center_x, decode_bbox_center_y;
  float decode_bbox_width, decode_bbox_height;
  float decode_bbox_radians;
  if (variance_encoded_in_target) {
    // variance is encoded in target, we simply need to retore the offset
    // predictions.
    decode_bbox_center_x = bbox.xc() * prior_width + prior_center_x;
    decode_bbox_center_y = bbox.yc() * prior_height + prior_center_y;
    decode_bbox_width = exp(bbox.width()) * prior_width;
    decode_bbox_height = exp(bbox.height()) * prior_height;
    decode_bbox_radians = bbox.radians();
  } else {
    // variance is encoded in bbox, we need to scale the offset accordingly.
    decode_bbox_center_x =
        prior_variance[0] * bbox.xc() * prior_width + prior_center_x;
    decode_bbox_center_y =
        prior_variance[1] * bbox.yc() * prior_height + prior_center_y;
    decode_bbox_width =
        exp(prior_variance[2] * bbox.width()) * prior_width;
    decode_bbox_height =
        exp(prior_variance[3] * bbox.height()) * prior_height;
    decode_bbox_radians = bbox.radians();
  }

  decode_bbox->set_xc(decode_bbox_center_x);
  decode_bbox->set_yc(decode_bbox_center_y);
  decode_bbox->set_width(decode_bbox_width);
  decode_bbox->set_height(decode_bbox_height);
  decode_bbox->set_radians(decode_bbox_radians);
}

void DecodeOrientedBBoxes(
    const vector<NormalizedBBox>& prior_bboxes,
    const vector<vector<float> >& prior_variances,
    const CodeType code_type, const bool variance_encoded_in_target,
    const bool clip_bbox, const vector<NormalizedOrientedBBox>& bboxes,
    vector<NormalizedOrientedBBox>* decode_bboxes) {
  CHECK_EQ(prior_bboxes.size(), prior_variances.size());
  CHECK_EQ(prior_bboxes.size(), bboxes.size());
  int num_bboxes = prior_bboxes.size();
  if (num_bboxes >= 1) {
  ...
  for (int i = 0; i < num_bboxes; ++i) {
    NormalizedOrientedBBox decode_bbox;
    DecodeOrientedBBox(prior_bboxes[i], prior_variances[i], code_type,
               variance_encoded_in_target, clip_bbox, bboxes[i], &decode_bbox);
    decode_bboxes->push_back(decode_bbox);
  }
}

void DecodeOrientedBBoxesAll(const vector<LabelOrientedBBox>& all_loc_preds,
    const vector<NormalizedBBox>& prior_bboxes,
    const vector<vector<float> >& prior_variances,
    const int num, const bool share_location,
    const int num_loc_classes, const int background_label_id,
    const CodeType code_type, const bool variance_encoded_in_target,
    const bool clip, vector<LabelOrientedBBox>* all_decode_bboxes) {
  CHECK_EQ(all_loc_preds.size(), num);
  all_decode_bboxes->clear();
  all_decode_bboxes->resize(num);
  for (int i = 0; i < num; ++i) {
    // Decode predictions into bboxes.
    LabelOrientedBBox& decode_bboxes = (*all_decode_bboxes)[i];
    for (int c = 0; c < num_loc_classes; ++c) {
      int label = share_location ? -1 : c;
      if (label == background_label_id) {
        // Ignore background class.
      ...
      const vector<NormalizedOrientedBBox>& label_loc_preds =
          all_loc_preds[i].find(label)->second;
      DecodeOrientedBBoxes(prior_bboxes, prior_variances,
                   code_type, variance_encoded_in_target, clip,
                   label_loc_preds, &(decode_bboxes[label]));
    }
  }
...
/////////
template <typename Dtype>
void GetGroundTruth(const Dtype* oriented_gt_data, const Dtype* gt_data, const int num_gt,
      const int background_label_id, const bool use_difficult_gt,
      map<int, vector<NormalizedOrientedBBox> >* all_gt_bboxes) {
  all_gt_bboxes->clear();
  for (int i = 0; i < num_gt; ++i) {
    int start_idx = i * 8;
    int start_idx_oriented = i * 5;
    int item_id = gt_data[start_idx];
    if (item_id == -1) {
      continue;
    }
    ...
    NormalizedOrientedBBox bbox;
    bbox.set_xc(oriented_gt_data[start_idx_oriented + 0]);
    bbox.set_yc(oriented_gt_data[start_idx_oriented + 1]);
    bbox.set_width(oriented_gt_data[start_idx_oriented + 2]);
    bbox.set_height(oriented_gt_data[start_idx_oriented + 3]);
    bbox.set_radians(oriented_gt_data[start_idx_oriented + 4]);
    (*all_gt_bboxes)[item_id].push_back(bbox);
  }
}

// Explicit initialization.
template void GetGroundTruth(const double* oriented_gt_data, const double* gt_data, const int num_gt,
      const int background_label_id, const bool use_difficult_gt,
      map<int, vector<NormalizedOrientedBBox> >* all_gt_bboxes);
template void GetGroundTruth(const float* oriented_gt_data, const float* gt_data, const int num_gt,
      const int background_label_id, const bool use_difficult_gt,
      map<int, vector<NormalizedOrientedBBox> >* all_gt_bboxes);
...
void GetGroundTruth(const Dtype* oriented_gt_data, const Dtype* gt_data, const int num_gt,
      const int background_label_id, const bool use_difficult_gt,
      map<int, LabelOrientedBBox>* all_gt_bboxes) {
  all_gt_bboxes->clear();
  for (int i = 0; i < num_gt; ++i) {
    int start_idx = i * 8;
    int start_idx_oriented = i * 5;
...
template <typename Dtype>
void GetLocPredictions(const Dtype* oriented_loc_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelOrientedBBox>* oriented_loc_preds) {
  oriented_loc_preds->clear();
  if (share_location) {
    CHECK_EQ(num_loc_classes, 1);
  }
  oriented_loc_preds->resize(num);
  for (int i = 0; i < num; ++i) {
    LabelOrientedBBox& label_bbox = (*oriented_loc_preds)[i];
    for (int p = 0; p < num_preds_per_class; ++p) {
      int start_idx = p * num_loc_classes * 5;
      for (int c = 0; c < num_loc_classes; ++c) {
        int label = share_location ? -1 : c;
        if (label_bbox.find(label) == label_bbox.end()) {
          label_bbox[label].resize(num_preds_per_class);
        }
        label_bbox[label][p].set_xc(oriented_loc_data[start_idx + c * 5]);
        label_bbox[label][p].set_yc(oriented_loc_data[start_idx + c * 5 + 1]);
        label_bbox[label][p].set_width(oriented_loc_data[start_idx + c * 5 + 2]);
        label_bbox[label][p].set_height(oriented_loc_data[start_idx + c * 5 + 3]);
        label_bbox[label][p].set_radians(oriented_loc_data[start_idx + c * 5 + 4]);
      }
    }
    oriented_loc_data += num_preds_per_class * num_loc_classes * 5;
  }
}
...
template void GetLocPredictions(const double* loc_data, const int num,
      ...
      const bool share_location, vector<LabelBBox>* loc_preds);
// Explicit initialization.
template void GetLocPredictions(const float* oriented_loc_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelOrientedBBox>* loc_preds);
template void GetLocPredictions(const double* oriented_loc_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelOrientedBBox>* loc_preds);
...
template <typename Dtype>
void EncodeOrientedLocPrediction(const vector<LabelOrientedBBox>& all_oriented_loc_preds,
      const map<int, vector<NormalizedOrientedBBox> >& all_oriented_gt_bboxes,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      Dtype* oriented_loc_pred_data, Dtype* oriented_loc_gt_data) {
  int num = all_oriented_loc_preds.size();
  // CHECK_EQ(num, all_match_indices.size());
  // Get parameters.
  const CodeType code_type = multibox_loss_param.code_type();
  const bool encode_variance_in_target =
  ...
  for (int i = 0; i < num; ++i) {
    for (map<int, vector<int> >::const_iterator
      ...
      const int label = it->first;
      const vector<int>& match_index = it->second;
      CHECK(all_oriented_loc_preds[i].find(label) != all_oriented_loc_preds[i].end());
      const vector<NormalizedOrientedBBox>& loc_pred =
          all_oriented_loc_preds[i].find(label)->second;
      for (int j = 0; j < match_index.size(); ++j) {
        if (match_index[j] <= -1) {
          continue;
        }
        // Store encoded ground truth.
        const int gt_idx = match_index[j];
        CHECK(all_oriented_gt_bboxes.find(i) != all_oriented_gt_bboxes.end());
        CHECK_LT(gt_idx, all_oriented_gt_bboxes.find(i)->second.size());
        const NormalizedOrientedBBox& gt_bbox = all_oriented_gt_bboxes.find(i)->second[gt_idx];
        NormalizedOrientedBBox gt_encode;
        CHECK_LT(j, prior_bboxes.size());
        bool flag = false;
        if(gt_bbox.width() >0 && gt_bbox.height() > 0){
          flag = true;
          EncodeOrientedBBox(prior_bboxes[j], prior_variances[j], code_type,
                     encode_variance_in_target, gt_bbox, &gt_encode);
          oriented_loc_gt_data[count * 5] = gt_encode.xc();
          oriented_loc_gt_data[count * 5 + 1] = gt_encode.yc();
          oriented_loc_gt_data[count * 5 + 2] = gt_encode.width();
          oriented_loc_gt_data[count * 5 + 3] = gt_encode.height();
          oriented_loc_gt_data[count * 5 + 4] = gt_encode.radians();
        }
        // Store location prediction.
        CHECK_LT(j, loc_pred.size());
        if (bp_inside) {
          NormalizedOrientedBBox match_bbox;
          if (!use_prior_for_matching) {
            const bool clip_bbox = false;
            DecodeOrientedBBox(prior_bboxes[j], prior_variances[j], code_type,
                       encode_variance_in_target, clip_bbox, loc_pred[j],
                       &match_bbox);
          }
          // When a dimension of match_bbox is outside of image region, use
          // gt_encode to simulate zero gradient.
          oriented_loc_pred_data[count * 5] =
              (match_bbox.xc() < 0 || match_bbox.xc() > 1) ?
              gt_encode.xc() : loc_pred[j].xc();
          oriented_loc_pred_data[count * 5 + 1] =
              (match_bbox.yc() < 0 || match_bbox.yc() > 1) ?
              gt_encode.yc() : loc_pred[j].yc();
          oriented_loc_pred_data[count * 5 + 2] =
              (match_bbox.width() < 0 || match_bbox.width() > 1) ?
              gt_encode.width() : loc_pred[j].width();
          oriented_loc_pred_data[count * 5 + 3] =
              (match_bbox.height() < 0 || match_bbox.height() > 1) ?
              gt_encode.height() : loc_pred[j].height();
          oriented_loc_pred_data[count * 5 + 4] = loc_pred[j].radians();
        } else {
          oriented_loc_pred_data[count * 5] = loc_pred[j].xc();
          oriented_loc_pred_data[count * 5 + 1] = loc_pred[j].yc();
          oriented_loc_pred_data[count * 5 + 2] = loc_pred[j].width();
          oriented_loc_pred_data[count * 5 + 3] = loc_pred[j].height();
          oriented_loc_pred_data[count * 5 + 4] = loc_pred[j].radians();
        }
        if (encode_variance_in_target) {
          for (int k = 0; k < 4; ++k) {
            CHECK_GT(prior_variances[j][k], 0);
            oriented_loc_pred_data[count * 5 + k] /= prior_variances[j][k];
            oriented_loc_gt_data[count * 5 + k] /= prior_variances[j][k];
          }
        }
        if (flag == false){
          oriented_loc_gt_data[count * 5] = loc_pred[j].xc();
          oriented_loc_gt_data[count * 5 + 1] = loc_pred[j].yc();
          oriented_loc_gt_data[count * 5 + 2] = loc_pred[j].width();
          oriented_loc_gt_data[count * 5 + 3] = loc_pred[j].height();
          oriented_loc_gt_data[count * 5 + 4] = loc_pred[j].radians();
        }
        //  LOG(INFO) << gt_bbox.xc() << " "
        //  << gt_bbox.yc() << " "
        //  << gt_bbox.width() << " "
        ...
        // << gt_bbox.radians() << " gt"
        //  << std::endl;
        // LOG(INFO) << loc_pred[j].xc() << " "
        //  << loc_pred[j].yc() << " "
        //  << loc_pred[j].width() << " "
        //  << loc_pred[j].height() << " "
        // << loc_pred[j].radians() << " pred"
        //  << std::endl;
        // LOG(INFO) << prior_bboxes[j].xmin() << " "
        //  << prior_bboxes[j].xmax() << " "
        //  << prior_bboxes[j].ymin() << " "
        //  << prior_bboxes[j].ymax() << " anchor"
        //  << std::endl;
        // LOG(INFO) << prior_variances[j][0] << " "
        //  << prior_variances[j][1] << " "
        //  << prior_variances[j][2] << " "
        //  << prior_variances[j][3] << " variance"
        //  << std::endl;
        // LOG(INFO) << gt_encode.xc() << " "
        //  << gt_encode.yc() << " "
        //  << gt_encode.width() << " "
        //  << gt_encode.height() << " "
        // << gt_encode.radians() << " "
...
// Explicit initialization.
template void EncodeOrientedLocPrediction(const vector<LabelOrientedBBox>& all_loc_preds,
      const map<int, vector<NormalizedOrientedBBox> >& all_gt_bboxes,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      float* oriented_loc_pred_data, float* oriented_loc_gt_data);
template void EncodeOrientedLocPrediction(const vector<LabelOrientedBBox>& all_loc_preds,
      const map<int, vector<NormalizedOrientedBBox> >& all_gt_bboxes,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      double* oriented_loc_pred_data, double* oriented_loc_gt_data);
