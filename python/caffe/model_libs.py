...
def Inception(net, from_layer, out_layer, use_bn, use_relu, use_scale=True, eps=0.001,
    conv_prefix='', conv_postfix='', bn_prefix='', bn_postfix='_bn',
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias',
    pool_prefix='', pool_postfix='_pool'):

  #pool_name = '{}{}{}'.format(conv_prefix, out_layer, pool_postfix)

  conv_name_reduction = '{}{}{}_reduction'.format(conv_prefix, out_layer, conv_postfix)
  ConvBNLayer(net, from_layer, conv_name_reduction, use_bn, use_relu, 128, 1, 0, 1)

  conv_name_3x3 = '{}{}{}_3x3'.format(conv_prefix, out_layer, conv_postfix)
  ConvBNLayer(net, from_layer, conv_name_3x3, use_bn, use_relu, 128, 3, 2, 1, 2)

  conv_name_3x3_pool = '{}{}{}_3x3_pool'.format(conv_prefix, out_layer, conv_postfix)
  conv_name_1x1 = '{}{}{}_1x1'.format(conv_prefix, out_layer, conv_postfix)
  net[conv_name_3x3_pool] = L.Pooling(net[from_layer], pool=P.Pooling.MAX, kernel_size=3, pad=1, stride=1)
  ConvBNLayer(net, conv_name_3x3_pool, conv_name_1x1, use_bn, use_relu, 128, 1, 0, 1)

  conv_name_1x5 = '{}{}{}_1x5'.format(conv_prefix, out_layer, conv_postfix)
  conv_name_5x1 = '{}{}{}_5x1'.format(conv_prefix, out_layer, conv_postfix)
  ConvBNLayerMXN(net, from_layer, conv_name_1x5, use_bn, use_relu, 128, 1, 5, 0, 4, 1, 1, 2)
  ConvBNLayerMXN(net, conv_name_1x5, conv_name_5x1, use_bn, use_relu, 128, 5, 1, 4, 0, 1, 1, 2)

  net[out_layer] = L.Concat(net[conv_name_reduction], net[conv_name_3x3], net[conv_name_1x1], net[conv_name_5x1])  # 320

def Inception_ONE(net, from_layer, out_layer, use_bn, use_relu, use_scale=True, eps=0.001,
    conv_prefix='', conv_postfix='', bn_prefix='', bn_postfix='_bn',
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias',
    pool_prefix='', pool_postfix='_pool'):
  ...
  conv_name_1x5 = '{}{}{}_1x5'.format(conv_prefix, out_layer, conv_postfix)
  conv_name_5x1 = '{}{}{}_5x1'.format(conv_prefix, out_layer, conv_postfix)
  ConvBNLayerMXN(net, from_layer, conv_name_1x5, use_bn, use_relu, 128, 1, 5, 0, 2, 1, 1, 1)
  ConvBNLayerMXN(net, conv_name_1x5, conv_name_5x1, use_bn, use_relu, 128, 5, 1, 2, 0, 1, 1, 1)

  net[out_layer] = L.Concat(net[conv_name_reduction], net[conv_name_3x3], net[conv_name_1x1], net[conv_name_5x1])  # 320


def ConvBNLayerMXN(net, from_layer, out_layer, use_bn, use_relu, num_output,
    kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, dilation=1, use_scale=True, eps=0.001, lr_mult=1,
    conv_prefix='', conv_postfix='', bn_prefix='', bn_postfix='_bn',
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias',
    **bn_params):
  if use_bn:
...
def ConvBNLayer(net, from_layer, out_layer, use_bn, use_relu, num_output,
    kernel_size, pad, stride, dilation=1, use_scale=True, eps=0.001,lr_mult=1,
    conv_prefix='', conv_postfix='', bn_prefix='', bn_postfix='_bn',
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias'):
  if use_bn:
    # parameters for convolution layer with batchnorm.
...
def CreateAnnotatedDataMaskLayer(source, batch_size=32, backend=P.Data.LMDB,
        output_label=True, train=True, label_map_file='',
        transform_param={}, mask_param={}, batch_sampler=[{}], oriented=False):
    if train:
        kwargs = {
                'include': dict(phase=caffe_pb2.Phase.Value('TRAIN')),
                'transform_param': transform_param,
    ...
    if output_label:
        data, mask, label = L.AnnotatedDataMask(name="data",
            annotated_data_param=dict(label_map_file=label_map_file,
                batch_sampler=batch_sampler, oriented=oriented),
            data_param=dict(batch_size=batch_size, backend=backend, source=source),
            ntop=3, **kwargs)
        return [data, mask, label]
    else:
        data = L.AnnotatedDataMask(name="data",
            annotated_data_param=dict(label_map_file=label_map_file,
                batch_sampler=batch_sampler, oriented=oriented),
            data_param=dict(batch_size=batch_size, backend=backend, source=source),
            ntop=1, **kwargs)
...
def CreateMultiBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
    ...
    for i in range(0, num):
        ...
        ResBody(net, from_layer, from_layer, out2a=256, out2b=256, out2c=512, stride=1, use_branch1=True)
        from_layer = 'res'+from_layer
...
def CreateMultiBoxRotationHead(net, data_layer="data", num_classes=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1,
        use_scale=True, min_sizes=[], max_sizes=[], prior_variance = [0.1],
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0,
        conf_postfix='', loc_postfix='', oriented_postfix='', append_layer=[], **bn_param):
    assert num_classes, "must provide num_classes"
    assert num_classes > 0, "num_classes must be positive number"
    if normalizations:
        assert len(from_layers) == len(normalizations), "from_layers and normalizations should have same length"
    ...
    for i in range(0, num):
        ...
        # Create oriented location prediction layer.
        name = "{}_mbox_oriented{}".format(from_layer, oriented_postfix)
        num_oriented_output = num_priors_per_location * 5;
        if not share_location:
            num_oriented_output *= num_classes
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_oriented_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
        permute_name = "{}_perm".format(name)
        net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
        flatten_name = "{}_flat".format(name)
        net[flatten_name] = L.Flatten(net[permute_name], axis=1)
        oriented_layers.append(net[flatten_name])
    ...
    for layer in append_layer:
        mbox_layers.append(layer)
    name = "mbox_oriented"
    net[name] = L.Concat(*oriented_layers, axis=1)
    mbox_layers.append(net[name])
    return mbox_layers
