...
// storing them as AnnotatedMaskDatum proto buffers.
// Usage:
//   convert_annoset [FLAGS] ROOTFOLDER/ LISTFILE DB_NAME
//
// where ROOTFOLDER is the root folder that holds all the images and
// annotations, and LISTFILE should be a list of files as well as their label
// or mask files.

// For detection task, the file should be in the format as
//   imgfolder1/img1.JPEG annofolder1/anno1.xml maskfolder1/mask1.jpg
//   ....
...
int main(int argc, char** argv) {
  ...
  std::vector<boost::tuple<std::string, std::string, std::string> > lines;

  std::string filename;
  std::string labelname;
  std::string maskname;
  if (anno_type == "detection") {
    type = AnnotatedMaskDatum_AnnotationType_BBOX;
    LabelMap label_map;
    ...
    while (infile >> filename >> labelname >> maskname) {
      lines.push_back(boost::make_tuple(filename, labelname, maskname));
    }
  }
  if (FLAGS_shuffle) {
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    if (encoded && !enc.size()) {
      ...
      string fn = boost::get<0>(lines[line_id]);
      size_t p = fn.rfind('.');
      if ( p == fn.npos )
    ...
    filename = root_folder + boost::get<0>(lines[line_id]);
    if (anno_type == "detection") {
      labelname = root_folder + boost::get<1>(lines[line_id]);
      maskname = root_folder + boost::get<2>(lines[line_id]);
      status = ReadRichImageToAnnotatedMaskDatum(filename, labelname, maskname, resize_height,
          resize_width, min_dim, max_dim, is_color, enc, type, label_type,
          name_to_label, &anno_datum);
      anno_datum.set_type(AnnotatedMaskDatum_AnnotationType_BBOX);
    }
    if (status == false) {
      LOG(WARNING) << "Failed to read " << boost::get<0>(lines[line_id]);
      continue;
    }
    if (check_size) {
    ...
    // sequential
    string key_str = caffe::format_int(line_id, 8) + "_" + boost::get<0>(lines[line_id]);
